...
contract StateReceiver is System {
    ...
    function execute(
        ...
        StateSync calldata obj
    ) external {
        StateSyncCommitment memory commitment = getCommitmentByStateSyncId(
            obj.id
        );
    ...
    function batchExecute(
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            bool isMember = keccak256(abi.encode(objs[i]))
                .checkMembershipWithHeight(
     ...
     * @param id state sync to get the root for
     */
    function getCommitmentByStateSyncId(
        uint256 id
    ) public view returns (StateSyncCommitment memory) {
        uint256 idx = commitmentIds.findUpperBound(id);

        require(
            idx != commitmentIds.length,
    ...
    function _executeStateSync(StateSync calldata obj) private {
        require(
            !processedStateSyncs[obj.id],
            "StateReceiver: STATE_SYNC_IS_PROCESSED"
        );
